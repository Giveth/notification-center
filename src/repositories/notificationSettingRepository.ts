...
export const createNotificationSettingsForNewUser = async (
    ...
    for (const notificationType of notificationTypes) {
        const notificationParent = notificationType?.isGroupParent ?
            notificationType :
            await findNotificationTypeParent(notificationType.categoryGroup as string)
        const payload: Partial<NotificationSetting> = {
            notificationType: notificationType,
            userAddress: user,
            allowEmailNotification: notificationParent?.emailDefaultValue !== undefined ? notificationParent?.emailDefaultValue : true,
